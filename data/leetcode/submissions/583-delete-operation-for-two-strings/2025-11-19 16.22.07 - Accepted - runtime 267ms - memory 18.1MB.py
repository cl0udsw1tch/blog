class Solution:
    def minDistance(self, word1: str, word2: str) -> int:
        ...
        s_m: index in word2
        x_m: deletion of word1[m]                   {word1[m+1:],word2[s_m:]}, => C_m = 1
             deletion of word2[m]                   {word1[m:],word2[s_m+1:]}, => C_m = 1
        ...
        if M==1:
            if word1[0] not in word2: return N+M
            return N-1
        if N==1:
            if word2[0] not in word1: return N+M
            return M-1

        dp=[[], [2*int(word1[-1] not in word2[n:]) + (N-n-1) for n in range(N)]]
        ...
        for m in range(M-2,-1,-1):
            ...
            for s_m in range(N-1,-1,-1):
                ...
                for x_m in X[:2]:
                    s_mp1=s_m+x_m[1]
                    mp=x_m[0]
                    f=1 + (dp[mp][s_mp1] if s_mp1<N else M-(mp))
                    F.append(f)
